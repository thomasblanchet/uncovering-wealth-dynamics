...
dir.create(here("graphs", "07-simulate-model-estate-tax-high"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-estate-tax-high"), showWarnings = FALSE)
...
for (i in 1:5) {
    ...
    for (yr in 1963:2100) {
        ...
        avg_national_income_yr <- avg_national_income %>% filter(year == min(yr, year_pivot)) %>% pull(avg)
        ...
        if (yr <= year_pivot) {
            ...
            simul_inheritance_yr <- simul_inheritance(
                data_simul = data_simul_new,
                estate_tax_fun = function(w) pmax(0, w - 1e5/avg_national_income_yr)
            )
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-estate-tax-high", "data_simul_estate_tax_high_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-estate-tax-high", "data_simul_estate_tax_high_effects.rds"))
...
data_simul_estate_tax_high_tabul   <- read_rds(here("work", "07-simulate-model-estate-tax-high", "data_simul_estate_tax_high_tabul.rds"))
data_simul_estate_tax_high_effects <- read_rds(here("work", "07-simulate-model-estate-tax-high", "data_simul_estate_tax_high_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_estate_tax_high_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(file.path("graphs", "07-simulate-model-estate-tax-high", "actual-simul-top1-estate-high.pdf"), height = 3, width = 5)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2030, xend = 2040, y = 0.42, yend = 0.372,
            color = "#4daf4a", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.45,
            label = "With <strong>100% estate tax rate above $100k</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>~1.5pp. lower</strong> in the long run",
            size = 3,
            fill = NA,
            color = "#4daf4a",
            label.color = NA) +
...
# ---------------------------------------------------------------------------- #
# Plot of the Pareto coefficient for a simple model
# ---------------------------------------------------------------------------- #

alpha0 <- 1.5

sigma <- 0.4
mu <- 0.5*sigma^2*(1 - 1.5)
delta <- 1/50

data_tax_simple_model <- seq(0, 0.1, 0.001) %>% map_dfr(function(rate) {
    pareto_annual <- uniroot(
        f = function(alpha) mu + 0.5*(alpha - 1)*sigma^2 - 1 - delta/alpha + (1 - rate) + delta/alpha,
        lower = 1,
        upper = 100,
        extendInt = "yes"
    )$root

    pareto_estate <- uniroot(
        f = function(alpha) mu + 0.5*(alpha - 1)*sigma^2 - 1 - delta/alpha + 1 + (1 - rate)^(alpha/delta)/(alpha/delta),
        lower = 1,
        upper = 100,
        extendInt = "yes"
    )$root

    return(tibble(rate = rate, pareto_annual = pareto_annual, pareto_estate = pareto_estate))
})

pdf(file.path("graphs", "07-simulate-model-estate-tax-high", "simple-model-inheritance.pdf"), height = 3, width = 4)
print(
    data_tax_simple_model %>% ggplot() + theme_bw() +
        geom_line(aes(x = rate, y = pareto_annual), color = "#F26419", size = 1.5) +
        geom_line(aes(x = rate, y = pareto_estate), color = "#2F4858", size = 1.5) +

        annotate("text", x = 0.055, y = 2.6, label = TeX("Annual tax only ($\\chi = 0$)"), color = "#F26419", size = 4) +
        annotate("text", x = 0.07, y = 1.75, label = TeX("Inheritance tax only ($\\tau = 0$)"), color = "#2F4858", size = 4) +

        scale_x_continuous(name = TeX("$\\tau$, $\\chi$ (tax rate, annual wealth tax equivalent)"),
            labels = percent_format(accuracy = 1), limits = c(0, 0.1), breaks = seq(0, 0.1, 0.01)) +
        scale_y_continuous(name = "Pareto coefficient", breaks = seq(1, 3, 0.2), limits = c(1.4, 2.8)) +
