...
dir.create(here("graphs", "07-simulate-model-labor-income"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-labor-income"), showWarnings = FALSE)
...
simul_param_income <- simul_param_income %>%
    group_by(p) %>%
    mutate(mean_labor_pre1978 = mean(mean_labor[year <= 1978])) %>%
    mutate(var_labor_pre1978 = mean(var_labor[year <= 1978])) %>%
    mutate(cov_labor_capital_pre1978 = mean(cov_labor_capital/sqrt(var_labor*var_capital))*sqrt(var_labor_pre1978*var_capital)) %>%
    mutate(cov_labor_gains_pre1978 = mean(cov_labor_gains/sqrt(var_labor*var_gains))*sqrt(var_labor_pre1978*var_gains)) %>%
    mutate(mean_labor = if_else(year <= 1978, mean_labor, mean_labor_pre1978)) %>%
    mutate(var_labor = if_else(year <= 1978, var_labor, var_labor_pre1978)) %>%
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-labor-income", "data_simul_labor_income_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-labor-income", "data_simul_labor_income_effects.rds"))
...
data_simul_labor_income_tabul   <- read_rds(here("work", "07-simulate-model-labor-income", "data_simul_labor_income_tabul.rds"))
data_simul_labor_income_effects <- read_rds(here("work", "07-simulate-model-labor-income", "data_simul_labor_income_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_labor_income_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-labor-income", "actual-simul-top1-labor-income.pdf"), height = 3, width = 4)
print(
    tabulations_combined %>%
        ...
        annotate("richtext", x = 2010, y = 0.475,
            label = "With <strong>1962-1978 labor income inequality</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>~6pp. lower</strong> in the long run",
            color = "#F26419",
            size = 3,
            fill = NA,
            label.color = NA) +
