...
dir.create(here("graphs", "07-simulate-model-gains"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-gains"), showWarnings = FALSE)
...
simul_param_income <- simul_param_income %>%
    ...
    mutate(cov_capital_gains_pre1978 = mean(cov_capital_gains/sqrt(var_capital*var_gains))*sqrt(var_capital*var_gains_pre1978)) %>%
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-gains", "data_simul_gains_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-gains", "data_simul_gains_effects.rds"))
...
data_simul_gains_tabul    <- read_rds(here("work", "07-simulate-model-gains", "data_simul_gains_tabul.rds"))
data_simul_gains_effects  <- read_rds(here("work", "07-simulate-model-gains", "data_simul_gains_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_gains_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-gains", "actual-simul-top1-gains.pdf"), height = 3, width = 6)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2020, xend = 2050, y = 0.45, yend = 0.297,
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.45,
            label = "With <strong>1962-1978 capital gains/losses</strong>,<br>the top 1% share would be<br><strong>~9pp. lower</strong> in the long run",
            size = 2.5) +
