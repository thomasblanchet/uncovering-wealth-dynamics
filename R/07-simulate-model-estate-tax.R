...
dir.create(here("graphs", "07-simulate-model-estate-tax"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-estate-tax"), showWarnings = FALSE)
...
for (i in 1:5) {
    ...
    for (yr in 1963:2100) {
        ...
        simul_inheritance_yr <- simul_inheritance(
            data_simul = data_simul_new,
            estate_tax_fun = function(w) estate_tax(w*avg_national_income_yr, min(yr, year_pivot))/avg_national_income_yr
        )
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-estate-tax", "data_simul_estate_tax_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-estate-tax", "data_simul_estate_tax_effects.rds"))
...
data_simul_estate_tax_tabul   <- read_rds(here("work", "07-simulate-model-estate-tax", "data_simul_estate_tax_tabul.rds"))
data_simul_estate_tax_effects <- read_rds(here("work", "07-simulate-model-estate-tax", "data_simul_estate_tax_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_estate_tax_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-estate-tax", "actual-simul-top1-estate-tax.pdf"), height = 3, width = 4)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2030, xend = 2040, y = 0.45, yend = 0.387,
            color = "#F26419", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.475,
            label = "With <strong>the 1978 estate tax schedule</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>similar</strong> in the long run",
            size = 3,
            fill = NA,
            color = "#F26419",
            label.color = NA) +
