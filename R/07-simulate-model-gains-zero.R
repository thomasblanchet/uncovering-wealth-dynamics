...
dir.create(here("graphs", "07-simulate-model-gains-zero"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-gains-zero"), showWarnings = FALSE)
...
# Counterfactual capital gains
simul_param_income <- simul_param_income %>%
    group_by(p) %>%
    mutate(mean_gains = if_else(year <= 1978, mean_gains, 0)) %>%
    mutate(var_gains = if_else(year <= 1978, var_gains, 0)) %>%
    mutate(cov_labor_gains = if_else(year <= 1978, cov_labor_gains, 0)) %>%
    mutate(cov_capital_gains = if_else(year <= 1978, cov_capital_gains, 0))
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-gains-zero", "data_simul_gains_zero_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-gains-zero", "data_simul_gains_zero_effects.rds"))
...
data_simul_gains_zero_tabul   <- read_rds(here("work", "07-simulate-model-gains-zero", "data_simul_gains_zero_tabul.rds"))
data_simul_gains_zero_effects <- read_rds(here("work", "07-simulate-model-gains-zero", "data_simul_gains_zero_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_gains_zero_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-gains-zero", "actual-simul-top1-gains-zero.pdf"), height = 3, width = 5)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2030, xend = 2050, y = 0.42, yend = 0.34,
            color = "#4daf4a", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.45,
            label = "With <strong>zero capital gains</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>~4pp. lower</strong> in the long run",
            size = 3,
            fill = NA,
            color = "#4daf4a",
            label.color = NA) +
