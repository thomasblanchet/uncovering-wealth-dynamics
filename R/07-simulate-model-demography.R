...
dir.create(here("graphs", "07-simulate-model-demography"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-demography"), showWarnings = FALSE)
...
# Estimate counterfactual death rates and population growth

# Death rates in 1962-1978
life_table_1962_1978 <- life_table %>%
    filter(year %in% c(1962:1978)) %>%
    group_by(sex, age) %>%
    summarise(qx = mean(qx))

# Birth rates in 1962-1978
fertility_1962_1978 <- fertility %>%
    filter(year %in% c(1962:1978)) %>%
    group_by(age) %>%
    summarise(asfr = mean(asfr))

# Alternative population structure
population_counterfactual <- population %>% filter(year == 1978) %>%
    select(-source, -variant) %>%
    ungroup()
for (yr in 1979:2100) {
    population_counterfactual_yr <- population_counterfactual %>% filter(year == yr - 1)
    population_counterfactual_yr <- population_counterfactual_yr %>%
        left_join(life_table_1962_1978, by = c("sex", "age")) %>%
        left_join(fertility_1962_1978, by = c("age")) %>%
        mutate(asfr = replace_na(asfr, 0)) %>%
        mutate(qx = replace_na(qx, 1))

    # Births by sex
    population_counterfactual_yr <- population_counterfactual_yr %>%
        mutate(sex_ratio_birth = population[age == 0 & sex == "male"] /
            (population[age == 0 & sex == "male"] + population[age == 0 & sex == "female"])) %>%
        mutate(birth_male = if_else(sex == "female", population*asfr*sex_ratio_birth, NA_real_)) %>%
        mutate(birth_female = if_else(sex == "female", population*asfr*(1 - sex_ratio_birth), NA_real_)) %>%
        mutate(birth_male = sum(birth_male, na.rm = TRUE), birth_female = sum(birth_female, na.rm = TRUE))

    # Population following year
    population_counterfactual_yr <- population_counterfactual_yr %>%
        group_by(sex) %>%
        arrange(sex, age) %>%
        mutate(population = case_when(
            age == 0 & sex == "male" ~ birth_male,
            age == 0 & sex == "female" ~ birth_female,
            TRUE ~ lag(population)*(1 - lag(qx))
        )) %>%
        ungroup()

    population_counterfactual <- bind_rows(
        population_counterfactual,
        population_counterfactual_yr %>% transmute(year = yr, age, sex, population)
    )
}

# Retrieve crude death rates and crude population growth
pop_death_counterfactual <- population_counterfactual %>%
    left_join(life_table_1962_1978) %>%
    group_by(year) %>%
    summarise(death = sum(population*qx), population = sum(population)) %>%
    arrange(year) %>%
    transmute(year, qx = death/population, pop_growth = population/lag(population) - 1) %>%
    filter(year > 1978)
...
for (i in 1:5) {
    ...
    for (yr in 1963:2100) {
        ...
        if (yr <= year_pivot) {
            ...
            death_rate <- pop_death_counterfactual %>% filter(year == yr) %>% pull(qx)
            growth_rate <- pop_death_counterfactual %>% filter(year == yr) %>% pull(pop_growth)
        }
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-demography", "data_simul_demography_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-demography", "data_simul_demography_effects.rds"))
...
data_simul_demography_tabul   <- read_rds(here("work", "07-simulate-model-demography", "data_simul_demography_tabul.rds"))
data_simul_demography_effects <- read_rds(here("work", "07-simulate-model-demography", "data_simul_demography_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_demography_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-demography", "actual-simul-top1-demography.pdf"), height = 3, width = 5)
print(
    tabulations_combined %>%
        ...
        annotate("text", label = "Observed", x = 1918, y = 0.28, color = "#e41a1c", size = 3) +
        annotate("segment", x = 1930, xend = 1944, y = 0.28, yend = 0.31, color = "#e41a1c",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +

        annotate("segment", x = 2030, xend = 2040, y = 0.425, yend = 0.387,
            color = "#4daf4a", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.45,
            label = "With <strong>1962-1978 demography</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>similar</strong> in the long run",
            color = "#4daf4a",
            size = 3,
            fill = NA,
            label.color = NA) +
