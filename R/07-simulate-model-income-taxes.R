...
dir.create(here("graphs", "07-simulate-model-income-taxes"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-income-taxes"), showWarnings = FALSE)
...
# Counterfactual tax rates by g-percentile
dina_tax_rates <- dina_micro %>%
    ...
    group_by(year) %>%
    complete(bracket = full_seq(bracket, 1)) %>%
    ...
    mutate(rate_l = approx(x = p, y = rate_l, rule = 2, xout = p)$y) %>%
    mutate(rate_k = approx(x = p, y = rate_k, rule = 2, xout = p)$y) %>%

    group_by(p) %>%
    mutate(rate_l_pre1978 = mean(rate_l[year <= 1978])) %>%
    mutate(rate_k_pre1978 = mean(rate_k[year <= 1978])) %>%

    select(year, p, rate_l, rate_k, rate_l_pre1978, rate_k_pre1978)

# Counterfactual labor income
simul_param_income <- simul_param_income %>%
    left_join(dina_tax_rates) %>%
    mutate(mean_labor = if_else(year <= 1978, mean_labor, mean_labor - pmax(mean_pre_labor, 0)*(rate_l - rate_l_pre1978))) %>%
    mutate(mean_capital = if_else(year <= 1978, mean_capital, mean_capital - pmax(mean_pre_capital, 0)*(rate_k - rate_k_pre1978))) %>%
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-income-taxes", "data_simul_income_taxes_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-income-taxes", "data_simul_income_taxes_effects.rds"))
...
data_simul_income_taxes_tabul   <- read_rds(here("work", "07-simulate-model-income-taxes", "data_simul_income_taxes_tabul.rds"))
data_simul_income_taxes_effects <- read_rds(here("work", "07-simulate-model-income-taxes", "data_simul_income_taxes_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_income_taxes_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-income-taxes", "actual-simul-top1-income-taxes.pdf"), height = 3, width = 4)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2030, xend = 2050, y = 0.45, yend = 0.345,
            color = "#F26419", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.475,
            label = "With <strong>1962-1978 tax rates</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>~3pp. lower</strong> in the long run",
            color = "#F26419",
            size = 3,
            fill = NA,
            label.color = NA) +
