...
dir.create(here("graphs", "07-simulate-model-growth"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-growth"), showWarnings = FALSE)
...
for (i in 1:5) {
    ...
    for (yr in 1963:2100) {
        ...
        if (yr <= year_pivot) {
            ...
            growth_differential <- (growth_rates %>% filter(year %in% c(1962:1978)) %>% pull(growth_rate) %>% mean()) -
                growth_rates %>% filter(year %in% c(2010:2019)) %>% pull(growth_rate) %>% mean()
        ...
        growth_rate_yr <- growth_rate_yr + growth_differential
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-growth", "data_simul_growth_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-growth", "data_simul_growth_effects.rds"))
...
data_simul_growth_tabul    <- read_rds(here("work", "07-simulate-model-growth", "data_simul_growth_tabul.rds"))
data_simul_growth_effects  <- read_rds(here("work", "07-simulate-model-growth", "data_simul_growth_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_growth_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-growth", "actual-simul-top1-growth.pdf"), height = 3, width = 4)
print(
    tabulations_combined %>%
        ...
        annotate("segment", x = 2030, xend = 2050, y = 0.45, yend = 0.35,
            color = "#F26419", arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("richtext", x = 2010, y = 0.475,
            label = "With <strong>1962-1978 economic growth</strong>,<br>the <strong>top 1% share</strong> would be<br><strong>~3pp. lower</strong> in the long run",
            color = "#F26419",
            size = 3,
            fill = NA,
            label.color = NA) +
