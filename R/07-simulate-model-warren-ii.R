...
dir.create(here("graphs", "07-simulate-model-warren-ii"), showWarnings = FALSE)
dir.create(here("work", "07-simulate-model-warren-ii"), showWarnings = FALSE)
...
write_rds(data_simul_tabul,   here("work", "07-simulate-model-warren-ii", "data_simul_warren2_tabul.rds"))
write_rds(data_simul_effects, here("work", "07-simulate-model-warren-ii", "data_simul_warren2_effects.rds"))
...
data_simul_warren1_tabul   <- read_rds(here("work", "07-simulate-model-warren-ii", "data_simul_warren2_tabul.rds"))
data_simul_warren1_effects <- read_rds(here("work", "07-simulate-model-warren-ii", "data_simul_warren2_effects.rds"))
...
tabulations_combined <- bind_rows(
    ...
    data_simul_warren2_tabul %>% mutate(type = "Simulated\n(counterfactual)"),
...
pdf(here("graphs", "07-simulate-model-warren-ii", "actual-simul-top1-warren-ii.pdf"), height = 3, width = 6)
print(
...
# ---------------------------------------------------------------------------- #
# Compare the three wealth taxes
# ---------------------------------------------------------------------------- #

data_simul_future_tabul  <- read_rds(here("work", "07-simulate-model-future", "data_simul_future_tabul.rds"))
data_simul_sanders_tabul <- read_rds(here("work", "07-simulate-model-sanders", "data_simul_sanders_tabul.rds"))
data_simul_warren1_tabul <- read_rds(here("work", "07-simulate-model-warren-i", "data_simul_warren1_tabul.rds"))
data_simul_warren2_tabul <- read_rds(here("work", "07-simulate-model-warren-ii", "data_simul_warren2_tabul.rds"))
...
tabulations_combined <- bind_rows(
    model_micro_data %>%
        ...
        arrange(p) %>%
        mutate(type = "Observed"),
    data_simul_warren2_tabul %>% mutate(type = "Warren\n(second version)") %>% filter(year > 2015),
    data_simul_warren1_tabul %>% mutate(type = "Warren\n(first version)") %>% filter(year > 2015),
    data_simul_sanders_tabul %>% mutate(type = "Sanders") %>% filter(year > 2015),
    data_simul_future_tabul %>% mutate(type = "No wealth tax")
)

# Add 1962 to simulation group (using real value, as this is the starting point)
tabulations_combined <- tabulations_combined %>% bind_rows(
    tabulations_combined %>% filter(year == 1962 & type == "Observed") %>% mutate(type = "No wealth tax")
)

pdf(here("graphs", "07-simulate-model-warren-ii", "actual-simul-top1-wealth-taxes.pdf"), height = 3, width = 5)
print(
    tabulations_combined %>%
        filter(p == 99000) %>%
        group_by(type, year) %>%
        ...
        summarise(year = mean(year), ts = median(ts)) %>%
        group_by(year) %>%
        mutate(ts = if_else(year == 2018 & type != "Observed", mean(ts), ts)) %>%
        ggplot() +
        theme_bw() +
        geom_line(aes(x = year, y = ts, color = type), size = 0.7) +
        geom_point(aes(x = year, y = ts, color = type, shape = type), size = 1.5) +

        annotate("segment", x = 1985, xend = 1992, y = 0.313, yend = 0.295, color = "#969A97",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("text", x = 1980, y = 0.32, label = "Observed", color = "#969A97", size = 3) +

        annotate("segment", x = 1998, xend = 1992, y = 0.238, yend = 0.265, color = "#2F4858",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("text", x = 2000, y = 0.225, label = "Simulation\n(no wealth tax)", color = "#2F4858", size = 3) +

        annotate("segment", x = 2050, xend = 2045, y = 0.395, yend = 0.361, color = "#F26419",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("text", x = 2050, y = 0.41, label = "With Warren tax\n(first version)", color = "#F26419", size = 3) +

        annotate("segment", x = 2028, xend = 2035, y = 0.312, yend = 0.357, color = "#C04000",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("text", x = 2025, y = 0.30, label = "With Warren tax\n(second version)", color = "#C04000", size = 3) +

        annotate("segment", x = 2055, xend = 2050, y = 0.285, yend = 0.342, color = "#679436",
            arrow = arrow(length = unit(0.20, "cm"), ends = "last", type = "open")) +
        annotate("text", x = 2055, y = 0.28, label = "With Sanders tax", color = "#679436", size = 3) +

        scale_color_manual(values = c(
            `Observed` = "#969A97",
            `No wealth tax` = "#2F4858",
            `Warren\n(first version)` = "#F26419",
            `Warren\n(second version)` = "#C04000",
            `Sanders` = "#679436"
        )) +
        scale_shape_manual(values = c(`Observed` = 1, `Warren\n(first version)` = 0, `Warren\n(second version)` = 1, `Sanders` = 2, `No wealth tax` = 5),
            limits = c("Observed", "No wealth tax", "Warren\n(first version)", "Warren\n(second version)", "Sanders")) +
        scale_y_continuous(name = "Top 1% share", limits = c(0.2, 0.42), breaks = seq(0.2, 0.42, 0.05),
            minor_breaks = NULL, labels = label_percent(accuracy = 1)) +
        scale_x_continuous(name = NULL, limits = c(1960, 2070), breaks = seq(1960, 2070, 20), minor_breaks = NULL) +
